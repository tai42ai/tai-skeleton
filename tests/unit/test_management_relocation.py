"""tai42_skeleton ships its own ``tools/`` and ``hooks/`` feature packages but
...
``tai42_skeleton.tools.management`` module string must fail loudly — no compat
...
def test_old_management_module_string_fails_loudly() -> None:
    with pytest.raises(ModuleNotFoundError):
        importlib.import_module("tai42_skeleton.tools.management")
