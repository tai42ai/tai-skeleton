...
caller. The gate's own refusals (401/403) and the rate limiter's 429 are answered
above this middleware and write no line — the trail covers only requests the gate
ADMITS. The principal is READ from the gate-bound caller id
(:func:`tai42_contract.access_control.context.get_current_user_id`), never
re-resolved from the credential; no bound caller (public route, or gate off)
audits as ``anonymous``.
...
belongs to that app, which MAY register this middleware for itself. The mount is
...
no per-request switch: ``AUDIT_LOG_ENABLE=false`` leaves this middleware out of
the stack entirely.
"""
...
# The principal recorded when no caller is bound (public route, or gate off).
...
# it is written, not even truncated.
...
class AuditLogMiddleware:
    ...
    def _route(scope: Scope, entered: _MatchMarks) -> str:
        ...
        if _crossed_a_mount(entered, matched):
            ...
            return f"{prefix}/{_UNMATCHED_ROUTE}" if prefix else _UNMATCHED_ROUTE
    ...
    def _emit(self, scope: Scope, entered: _MatchMarks, status: int | None, duration_ms: int, ts: str) -> None:
        """Write the audit line without breaking the request on failure (module
        ...
        try:
            ...
            logger.info(
                ...
                get_current_user_id() or _ANONYMOUS,
                scope["method"],
                route,
