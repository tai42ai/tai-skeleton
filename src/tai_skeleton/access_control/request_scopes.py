...
:class:`~tai_skeleton.access_control.middleware.ResourceGuardMiddleware` binds
...
(:func:`tai_skeleton.authz.check.check`) reaches the identical decision the HTTP
...
_current_effective_scopes: ContextVar[tuple[str, ...] | None] = ContextVar("tai_current_effective_scopes", default=None)
_current_identity_claims: ContextVar[Mapping[str, Any] | None] = ContextVar("tai_current_identity_claims", default=None)
...
def set_request_effective_scopes(scopes: tuple[str, ...] | None) -> Token[tuple[str, ...] | None]:
    ...
    with :func:`~tai_contract.access_control.context.set_request_user_id`; pass the
...
def set_request_identity_claims(claims: Mapping[str, Any] | None) -> Token[Mapping[str, Any] | None]:
    ...
    paired with :func:`~tai_contract.access_control.context.set_request_user_id`;
