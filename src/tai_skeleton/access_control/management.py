...
- The POLICY RULES (scopes, route/pattern mappings, per-user policy bodies) live in
  Postgres — the :class:`~tai_skeleton.access_control.store.PostgresAccessControlStore`
...
- The api-key IDENTITY record (key hash → ``{user_id, description}`` plus its
  ...
  ``tai-identity-redis`` plugin for ``auth_providers=["redis"]``), reached ONLY through
...
from tai_contract.access_control.identity import ApiKeyIdentityProvider, IdentityProvider
