...
The exporters/importers reference the live subsystems through the ``tai_app``
...
def _export_manifest() -> dict[str, Any]:
    ...
    return tai_app.config.config_manager.read_manifest_preserved()
...
async def _import_env(payload: dict[str, str]) -> _SectionReport:
    ...
    config_manager = tai_app.config.config_manager
    try:
...
async def _export_templates() -> dict[str, str]:
    resource_manager = tai_app.storage.resource_manager
...
async def _import_templates(payload: dict[str, str]) -> _SectionReport:
    from tai_skeleton.template.path_guard import UnsafeTemplatePathError, safe_template_path
...
async def _export_schedules() -> Any:
    ...
    return await tai_app.tools.run_tool("backend_export_schedules", {})


async def _import_schedules(payload: Any) -> _SectionReport:
    return await tai_app.tools.run_tool("backend_import_schedules", {"schedules": payload})
...
async def _export_connector_catalog() -> dict[str, Any]:
    from tai_skeleton.connectors.store.backup import export_connector_catalog

    return await export_connector_catalog()


async def _import_connector_catalog(payload: dict[str, Any]) -> _SectionReport:
    from tai_skeleton.connectors.store.backup import import_connector_catalog

    return await import_connector_catalog(payload)


async def _export_connector_connections() -> list[dict[str, Any]]:
    from tai_skeleton.connectors.store.backup import export_connector_connections
...
async def _import_connector_connections(payload: list[dict[str, Any]]) -> _SectionReport:
    from tai_skeleton.connectors.store.backup import import_connector_connections
...
async def _export_versioned_documents() -> dict[str, Any]:
    from tai_skeleton.versioning.backup import export_versioned_documents

    return await export_versioned_documents()


async def _import_versioned_documents(payload: dict[str, Any]) -> _SectionReport:
    from tai_skeleton.versioning.backup import import_versioned_documents
