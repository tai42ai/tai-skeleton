...
from tai_skeleton.routers.metrics_settings import activate_multiproc_env, metrics_settings

logger = logging.getLogger(__name__)


async def get_metrics() -> Response:
    # Imported lazily: ``tai_skeleton.routers.prometheus`` imports
...
def _register_manifest_identity_provider(settings: AccessControlSettings) -> None:
    ...
    touches the unbound ``tai_app`` handle, like a webhook verifier) is not an
    ...
    for module in manifest.lifecycle_modules or []:
        try:
            ...
            # (e.g. one registering through the unbound tai_app) legitimately cannot
    ...
    raise RuntimeError(
        ...
        "tai_identity_redis.redis_api_key_provider) in the manifest lifecycle_modules."
    )
...
def create_app() -> FastAPI:
    ...
    from tai_skeleton.routers.prometheus import init_prometheus_multiproc_dir
