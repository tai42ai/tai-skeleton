...
from tai_contract.monitoring import MonitoringLevel, SpanKind
...
class BaseHooksManager(ABC):
    ...
    async def _check_condition(self, hook: HookParams, payload: dict[str, Any]) -> bool:
        ...
        with writer.start_span(name="hook_check_condition", kind=SpanKind.CHAIN):
            raw_condition = await tai_app.storage.resource_manager.render_by_id_or_content(
    ...
    async def _run_hook(hook: HookParams, payload: dict[str, Any]):
        ...
        with writer.start_span(name="hook_run_tool", kind=SpanKind.CHAIN):
            ...
            rendered_expr = await tai_app.storage.resource_manager.render_by_id_or_content(
                content=hook.expr,
            ...
            await tai_app.tools.run_tool(hook.tool, tool_input)
