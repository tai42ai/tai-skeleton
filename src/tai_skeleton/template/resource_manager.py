...
from tai_kit.net import fetch_url

from tai_skeleton.template.media import ContentPart, MediaBlock
from tai_skeleton.template.path_guard import safe_template_path
from tai_skeleton.template.settings import template_cache_settings
...
class ResourceManager:
    ...
    def provider(self) -> Storage:
        ...
        if self._provider is None:
            raise RuntimeError(
                ...
                "tai_app.storage.register_storage before fetching or rendering a "
                "stored resource."
            )
    ...
    async def _load_by_id(self, template_id: str) -> str:
        ...
        :class:`~tai_skeleton.template.path_guard.UnsafeTemplatePathError` instead of
    ...
    async def load_file(self, source: str) -> str | MediaBlock:
        ...
        """
        from tai_skeleton.template.file_loading import load_content
    ...
    async def normalize_media(self, source: str | bytes) -> ContentPart:
        ...
        if isinstance(source, bytes):
            from tai_skeleton.template.file_loading import detect_mime
