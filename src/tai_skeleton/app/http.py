...
class HttpSurface:
    ...
    def custom_route(
        ...
        *,
        ...
        and its coverage gate read. See :class:`tai_contract.app.facets.AppHttp`.
...
def http_surface() -> HttpSurface:
    ...
    as ``tai_app.http``; the concrete server exposes it as ``_http_surface``.
    """
    ...
    surface = getattr(tai_app, "_http_surface", None)
    if surface is None:
        surface = tai_app.http
