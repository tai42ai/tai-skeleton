...
``tai_contract.app`` sub-protocols.

Each facet is a thin view bound to the owning :class:`~tai_skeleton.app.server.TaiMCP`;
...
the concrete app satisfies ``tai_contract.app.TaiApp`` (every member partitioned
...
if TYPE_CHECKING:
    ...
    from tai_contract.presets import PresetBody, PresetStore
    ...
    from tai_skeleton.manifest import Manifest as ManifestImpl
...
class MonitoringFacet(_Facet):
    ...
    def register_monitoring(self, builder: Callable[..., Any] | None = None) -> Callable[..., Any]:
        from tai_skeleton.monitoring import register_monitoring
...
class ExtensionsFacet(_Facet):
    ...
    def validate_combo(self, combo: Sequence[ExtensionElement]) -> None:
        ...
        :class:`~tai_skeleton.exceptions.exceptions.TaiValidationError`
...
class PresetsFacet(_Facet):
    ...
    async def set_version_tags(self, name: str, version: int, tags: list[str]) -> None:
        ...
        :class:`~tai_contract.versioning.errors.DocumentVersionNotFoundError` for an
