...
Three thin adapters over operations in ``tai_skeleton.operations.tool_runs`` — no
...
* ``POST /api/tool-runs`` — body ``{tool_name, arguments}`` (parsed by the same
  ...
  task through the ``tai_app.tools.run_tool`` seam.
...
from tai_skeleton.operations.tool_runs import get_run as _get_run_op
from tai_skeleton.operations.tool_runs import list_tool_runs as _list_tool_runs_op
from tai_skeleton.operations.tool_runs import submit_run as _submit_run_op
