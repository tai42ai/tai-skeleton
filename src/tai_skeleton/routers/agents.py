...
Four doors, bound to the :class:`~tai_contract.agent.Agent` CONTRACT only (never
...
* ``POST /api/agents/{name}/runs`` — the agent's input (validated against that
  ...
  :class:`~tai_contract.agent.events.StreamEvent`, a terminal ``stream.end`` when
...
``tai_skeleton.operations.agents``; the two run doors are SSE streams
...
from tai_skeleton.operations.agents import list_agents as _list_agents_op
from tai_skeleton.operations.agents import list_spec_runnable_agents as _list_spec_runnable_agents_op
