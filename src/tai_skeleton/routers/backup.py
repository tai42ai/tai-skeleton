...
Three AUTHED doors over the live ``tai_app.backup`` registry. The registry lists
...
``tai_skeleton.operations.backup`` (import's envelope shape is validated here at
...
from tai_skeleton.operations.backup import _DOCUMENT_VERSION, _maybe_await, _registered_section_names
from tai_skeleton.operations.backup import import_backup as _import_backup_op
from tai_skeleton.operations.backup import list_sections as _list_sections_op
...
async def export_backup(request: Request) -> Response:
    ...
    for name in requested:
        try:
            payload = await _maybe_await(tai_app.backup.export_section(name))
