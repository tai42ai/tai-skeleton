...
persists, :func:`~tai_skeleton.config.secret_seal.seal_resolved_secrets` retags any
leaf that still equals a currently-resolved secret back to its ``!ENV`` marker, and
refuses (a :class:`~tai_skeleton.config.secret_seal.ResolvedSecretError`, a
...
surfaces as a :class:`~tai_skeleton.operations._broadcast.FleetBroadcastError`
...
from tai_skeleton.app.boot_rules import check_backend_needs_bus
from tai_skeleton.app.bus import FleetResult, LocalApplyResult, OpOutcome
from tai_skeleton.app.bus_settings import bus_settings
from tai_skeleton.app.reload_gate import reload_gate
from tai_skeleton.config.secret_seal import seal_resolved_secrets
from tai_skeleton.manifest import Manifest
from tai_skeleton.operations._broadcast import FleetBroadcastError, fleet_fanout, log_non_convergence
...
class _ManifestStore(Protocol):
    ...
    :class:`~tai_contract.config.manager.ConfigManager`."""
...
class ApplyResult:
    ...
    def fanout(self) -> dict[str, Any]:
        ...
        :func:`~tai_skeleton.operations._broadcast.apply_response` embeds under
...
class ConfigService:
    ...
    def from_app(cls) -> ConfigService:
        ...
        return cls(config_manager=tai_app.config.config_manager, admin=tai_app.admin, bus=instance.app.bus)
    ...
    def _seal_secrets(self, document: dict[str, Any], current_preserved: Mapping[str, Any]) -> None:
        ...
        *current_preserved* is the CURRENT manifest in its preserved view (``!ENV``
        ...
        resolved value, then :func:`~tai_skeleton.config.secret_seal.seal_resolved_secrets`
        ...
        :class:`~tai_skeleton.config.secret_seal.ResolvedSecretError` on a stranded
