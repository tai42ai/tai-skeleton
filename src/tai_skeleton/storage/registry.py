...
The skeleton ships **no** concrete :class:`~tai_contract.storage.Storage` backend:
...
:class:`~tai_skeleton.template.ResourceManager` built on it raises loudly the
moment it is used (never a silent no-op). A backend ships as a separate
``tai-contract``-only plugin, loaded by the manifest, that registers through the
