...
from tai_skeleton.operations import BadRequestError, NotFoundError, NotSupportedError, operation
from tai_skeleton.tools.binding import is_unknown_tool_error
...
async def create_schedule(tool_name: str, tool_kwargs: dict[str, Any], schedule_kwargs: dict[str, Any]) -> Any:
    ...
    try:
        return await tai_app.tools.run_tool(tool_name, arguments)
    except RuntimeError as exc:
...
async def delete_schedule(schedule_name: str) -> Any:
    if not await _scheduling_backend_present():
        raise NotSupportedError(_NO_BACKEND_MESSAGE)
    return await tai_app.tools.run_tool("backend_delete_schedule", {"name": schedule_name})
