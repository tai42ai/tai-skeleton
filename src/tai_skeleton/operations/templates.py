...
A thin skin over the app's resource manager (``tai_app.storage.resource_manager``):
...
async def list_templates() -> list[str]:
    ...
    return await tai_app.storage.resource_manager.list_resources()
...
async def upload_template(path: str, content: str) -> dict:
    ...
    await tai_app.storage.resource_manager.upload_template(path=key, content=content)
...
async def delete_template(path: str) -> dict:
    ...
    """
    key = _safe_key(path)
    await tai_app.storage.resource_manager.delete_template(key)
...
async def clear_templates_cache() -> dict:
    ...
    tai_app.storage.resource_manager.clear_cache()
    return {"cleared": True}
