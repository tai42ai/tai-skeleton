...
Two operations over the live ``tai_app.backup`` registry. The registry lists its
...
def _registered_section_names() -> set[str]:
    return {info.name for info in tai_app.backup.sections()}


@operation(summary="List backup sections", tags=["backup"])
async def list_sections() -> list:
    return [{"name": info.name, "secret": info.secret} for info in tai_app.backup.sections()]
...
async def import_backup(document: dict[str, Any], sections: list[str]) -> dict:
    ...
    for name in sections:
        ...
        if name not in document_sections:
            ...
            continue
        try:
            report = await _maybe_await(tai_app.backup.import_section(name, document_sections[name]))
